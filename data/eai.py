...
HABITAT_ACTION_SPACE_TOKENIZE = {
    k: v for k, v in zip(list(HABITAT_ACTION_SPACE.values()), list(VICUNA_ACTION_TOKENS.keys())[:len(HABITAT_ACTION_SPACE)])
}
...
class CLIPortTokenizer:
    ...
    def tokenize(self, act_pose: tuple) -> tuple:
        ...
        act_trans[0] = min(max(act_trans[0], self._BOUNDS[0, 0]), self._BOUNDS[0, 1])
        act_trans[1] = min(max(act_trans[1], self._BOUNDS[1, 0]), self._BOUNDS[1, 1])

        u = int(np.round((act_trans[1] - self._BOUNDS[1, 0]) / self._PIXEL_SIZE))
        v = int(np.round((act_trans[0] - self._BOUNDS[0, 0]) / self._PIXEL_SIZE))
        ## set u, v to bound if out of bound
        u = min(max(u, 0), self._RESOLUTIN[0] - 1)
        v = min(max(v, 0), self._RESOLUTIN[1] - 1)
        ## quat to eulerXYZ
        quaternion_wxyz = np.array([act_quat[3], act_quat[0], act_quat[1], act_quat[2]])
        ...
        z_rot = int((z_rot_cont + self._ZROT_EPSILLON) / (2 * np.pi / self._ZBINS))
...
_CLIPORT_ACTION_SPACE_U = {
    k: v for k, v in zip(range(_cliport_tokenizer.num_tokens_u), list(VICUNA_ACTION_TOKENS.keys())[len(HABITAT_ACTION_SPACE):len(HABITAT_ACTION_SPACE)+_cliport_tokenizer.num_tokens_u] )
}


_CLIPORT_ACTION_SPACE_U_REVERSE = {
    v: k for k, v in zip(range(_cliport_tokenizer.num_tokens_u), list(VICUNA_ACTION_TOKENS.keys())[len(HABITAT_ACTION_SPACE):len(HABITAT_ACTION_SPACE)+_cliport_tokenizer.num_tokens_u] )
}


_CLIPORT_ACTION_SPACE_V = {
    k: v for k, v in zip(range(_cliport_tokenizer.num_tokens_v), list(VICUNA_ACTION_TOKENS.keys())[len(HABITAT_ACTION_SPACE)+_cliport_tokenizer.num_tokens_u:len(HABITAT_ACTION_SPACE)+_cliport_tokenizer.num_tokens_u+_cliport_tokenizer.num_tokens_v] )
}


_CLIPORT_ACTION_SPACE_V_REVERSE = {
    v: k for k, v in zip(range(_cliport_tokenizer.num_tokens_v), list(VICUNA_ACTION_TOKENS.keys())[len(HABITAT_ACTION_SPACE)+_cliport_tokenizer.num_tokens_u:len(HABITAT_ACTION_SPACE)+_cliport_tokenizer.num_tokens_u+_cliport_tokenizer.num_tokens_v] )
}


_CLIPORT_ACTION_SPACE_ZROT = {
    k: v for k, v in zip(range(_cliport_tokenizer.num_tokens_z), list(VICUNA_ACTION_TOKENS.keys())[len(HABITAT_ACTION_SPACE)+_cliport_tokenizer.num_tokens_u+_cliport_tokenizer.num_tokens_v:len(HABITAT_ACTION_SPACE)+_cliport_tokenizer.num_tokens_u+_cliport_tokenizer.num_tokens_v+_cliport_tokenizer.num_tokens_z])
}


_CLIPORT_ACTION_SPACE_ZROT_REVERSE = {
    v: k for k, v in zip(range(_cliport_tokenizer.num_tokens_z), list(VICUNA_ACTION_TOKENS.keys())[len(HABITAT_ACTION_SPACE)+_cliport_tokenizer.num_tokens_u+_cliport_tokenizer.num_tokens_v:len(HABITAT_ACTION_SPACE)+_cliport_tokenizer.num_tokens_u+_cliport_tokenizer.num_tokens_v+_cliport_tokenizer.num_tokens_z])
}

_DUMMY_CLIPORT_ACTION = {
    'pose0': (
        _cliport_tokenizer._BOUNDS[:, 0],
        np.array([0., 0., 0., 1.])
    ),
    'pose1': (
        _cliport_tokenizer._BOUNDS[:, 0],
...
def _extract_between(lst, start, end, padding):
    ...
    lst = [padding] * max(0, -start) + lst + [padding] * max(0, end - len(lst) + 1)
...
def _process_object_feature(obj_list, max_obj_points, label_mapping_path=None, habitat_alignment=False):
    # label_mapping_path:
    ...
    for obj_pcd in obj_list:
        ...
        pcd_idxs = np.random.choice(len(obj_pcd), size=max_obj_points, replace=(len(obj_pcd) < max_obj_points))
        ...
        obj_pcd[:, 3:6] = obj_pcd[:, 3:6] * 2 - 1     # rgb normalize (saved pointcloud have rgb as uint8 / 255.0)
...
def prepare_object_feature_habitat(object_path, label_mapping_path, max_obj_points):
    # object_path:
    #   path to "objects.npy"
    # label_mapping_path:
    #   path to "matterport_category_mappings.tsv"
    ...
    return _process_object_feature(obj_list, max_obj_points, label_mapping_path, habitat_alignment=True)
